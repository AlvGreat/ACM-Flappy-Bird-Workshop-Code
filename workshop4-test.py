...
"""
AI configuration (fitness function)
Modify this in order to change the way the AI is trained
"""
PIPE_COLLIDE_POINTS = -1 # points added for bird hitting a pipe (should be negative)
PIPE_PASSED_POINTS = 1 # points the bird gets for passing a pipe 
OUT_OF_BOUNDS_POINTS = -1 # points added for bird going out of bounds (should be negative)
POINTS_PER_SECOND = 1 # points a bird gains for staying alive for a second 
...
def main(genomes, config):
    ...
    while run:
        ...
        for x, bird in enumerate(birds): 
            ...
            ge[x].fitness += (POINTS_PER_SECOND / 30) # divide by 30 since this loop runs 30 times/second
        ...
        for pipe in pipes: 
            for x, bird in enumerate(birds): 
                if pipe.collide(bird, win): 
                    ge[x].fitness -= PIPE_COLLIDE_POINTS # it's bad for the bird to hit the pipe
        ...
        if add_pipe: 
            score += 1
            for g in ge: 
                g.fitness += PIPE_PASSED_POINTS # it's good for birds to pass the pipe
        ...
        for x, bird in enumerate(birds): 
            if (bird.y + bird.img.get_height() >= 730 or bird.y < 0):
                ge[x].fitness -= OUT_OF_BOUNDS_POINTS
                birds.pop(x) 
                nets.pop(x) 
                ge.pop(x) 
